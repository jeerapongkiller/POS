                        ...
                        <div class="col-md-3">
                            <div class="card-box">
                                ...
                                <p>Total Sell Charge : {{$outlet->outletCharges->sum('charge')}} % <br>
                                    ...
                                    Last payment
                                    : {{$last_payment ? $last_payment->created_at->format('d-M-Y') : 'No payment found'}}
                                </p>
                                @if($due - $total_payment > 0)
                                    <a href="{{url('/new-payment/outlet='.$outlet->id)}}"
                                       class="btn btn-success btn-sm">Make Payment</a>
                                @endif
                                <a href="{{url('/outlet-id='.$outlet->id.'/payment-history')}}"
